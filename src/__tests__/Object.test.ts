...
test("clone", () => {
  expect(run("hoge={test:256};huga=hoge.clone;huga.test=1024;return hoge.test")).toBe(256);
});

test("self", () => {
  expect(run("obj=Object.clone;obj.def(test(),self.hoge=1);obj.test();return obj.hoge")).toBe(1);
  expect(
    run(
