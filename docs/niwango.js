...
(function(global, factory) {
	...
	var import_niwango_core = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
		/*!
		...
		*/
		(function(global, factory) {
			typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define(factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.NiwangoCore = factory());
		})(exports, (function() {
			"use strict";
			let execute$1;
			...
			const processArrayExpression = (script, scopes, trace) => {
				return script.elements.reduce((result, element) => {
					return [...result, execute$1(element, scopes, trace)];
				}, []);
			};
			...
			const Multiplication = (left, right) => {
				if (typeof left === "string") return left.repeat(format(right, "number"));
				return format(left, "number") * format(right, "number");
			};
			const Subtraction = (left, right) => {
				const rightNum = format(right, "number");
				if (rightNum === 0 && typeof left === "string" && left.match(/^(0|0x)?[0-9]+(\.[0-9]+)?$/)) return Number(left);
				return format(left, "number") - rightNum;
			};
			const Addition = (left, right) => {
				if (typeof left === "string" || typeof right === "string") return `${format(left, "string")}${format(right, "string")}`;
				return format(left, "number") + format(right, "number");
			};
			...
			const processBlockStatement = (script, scopes, trace) => {
				return script.body.reduce((_, item) => execute$1(item, scopes, trace), void 0);
			};
			...
			const typeGuard = {
				AST: (i) => !!i && typeof i === "object" && typeof i.type === "string",
				...
				AssignmentExpression: (i) => !!i && typeof i === "object" && i.type === "AssignmentExpression",
				ArrayExpression: (i) => !!i && typeof i === "object" && i.type === "ArrayExpression",
				ArrowFunctionExpression: (i) => !!i && typeof i === "object" && i.type === "ArrowFunctionExpression",
				...
				definedFunction: (i) => !!i && typeof i === "object" && i.type === "definedFunction",
			...
			const assign = (target, value, scopes, trace) => {
				...
				try {
					if (typeGuard.Identifier(target)) {
						for (const scope of scopes) if (scope[target.name] !== void 0) {
						...
						const key = target.computed ? execute$1(target.property, scopes, trace) : getName$1(target.property, scopes, trace);
						left[key] = value;
					}
				} catch (e) {
					if (e instanceof Error) console.error(`[assign] ${e.name}: ${e.message}`, target, value, scopes, trace);
				}
			};
			const initAssign = () => {
				setAssign(assign);
			...
			const getName = (target, scopes, trace) => {
				if (typeGuard.Identifier(target)) return target.name;
				else return execute$1(target, scopes, trace);
			};
			const initGetName = () => {
				setGetName(getName);
			};
			const resolve = (script, scopes, trace) => {
				try {
					if (typeGuard.Identifier(script)) {
						for (const scope of scopes) if (scope[script.name] !== void 0) return processResolveHook(scope, script.name);
					}
				} catch (e) {
					if (e instanceof Error) console.error(`[resolve] ${e.name}: ${e.message}`, script, scopes, trace);
				}
			};
			const processResolveHook = (scope, name) => {
				let value = scope[name];
				for (const hook of resultHook) value = hook(value);
				scope[name] = value;
				return value;
			};
			...
			const processDistance = (script, scopes, _, trace) => {
				const args = argumentParser$1(script.arguments, scopes, [
					"x1",
					"y1",
					"x2",
					"y2"
			...
			const processDump = (script, scopes, _object, trace) => {
				const arr = [];
				for (const argument of script.arguments) arr.push(structuredClone(execute$1(argument, scopes, trace)));
			...
			const processIf = (script, scopes, _, trace) => {
				...
				if (execute$1(args.when, scopes, trace)) return execute$1(args.then, scopes, trace);
				else return execute$1(args.else, scopes, trace);
			};
			...
			const processWhileKari$1 = (script, scopes, _, trace) => {
				...
				let loopCount = 0;
				while (execute$1(script.arguments[0], scopes, trace) && loopCount++ <= 1e4) execute$1(script.arguments[1], scopes, trace);
			};
			const functions = {
				dump: processDump,
			...
			const processCallExpression = (script, scopes, trace) => {
				...
				const callee = getName$1(isMemberExpression ? script.callee.property : script.callee, scopes, trace);
				const object = getThis(script, scopes, trace);
				const objectRef = object?.[callee];
				if (typeGuard.definedFunction(objectRef)) return processDefinedFunction(script, scopes, trace, objectRef, object);
				const objectCallRef = object?.[callee]?.call;
				if (typeGuard.definedFunction(objectCallRef)) return processDefinedFunction(script, scopes, trace, objectCallRef, object);
				const self = resolve({
					type: "Identifier",
					name: "self"
				}, scopes, trace);
				const selfRef = self?.[callee];
				if (typeGuard.definedFunction(selfRef)) return processDefinedFunction(script, scopes, trace, selfRef);
				const selfCallRef = self?.[callee]?.call;
				if (typeGuard.definedFunction(selfCallRef)) return processDefinedFunction(script, scopes, trace, selfCallRef);
				const prototype = resolvePrototype$1(getType(object), callee);
				if (prototype) return prototype(script, scopes, object, trace);
				const func = functions[callee];
				if (func) return func(script, scopes, object, trace);
				const definedFunc = definedFunctions[callee];
				if (definedFunc) return definedFunc(script, scopes, object, trace);
				throw new NotImplementedError(script, scopes);
			};
			const processDefinedFunction = (script, scopes, trace, func, object) => {
				if (func.isKari) return processDefinedKariFunction(script, scopes, trace, func);
				else return processDefinedNormalFunction(script, scopes, trace, func, object);
			};
			const processDefinedKariFunction = (script, scopes, trace, func) => {
				const args = {};
				let count = 1;
				script.arguments.forEach((val) => {
					if (val?.NIWANGO_Identifier) args[getName$1(val.NIWANGO_Identifier, scopes, trace)] = execute$1(val, scopes, trace);
					else args[`$${count++}`] = execute$1(val, scopes, trace);
				});
			...
			const processDefinedNormalFunction = (script, scopes, trace, func, object) => {
				const argNames = func.script.arguments[0].arguments.map((arg) => getName$1(arg, scopes, trace));
				const args = argumentParser$1(script.arguments, scopes, argNames, trace);
				const scope = object ? [
					{
						...args,
						self: object
			...
			const processIdentifier = (script, scopes, trace) => {
				...
				if (typeGuard.definedFunction(value)) return execute$1(value.script.arguments[1], [{}, ...scopes], trace);
				if (value === void 0) try {
			...
			const processLogicalExpression = (script, scopes, trace) => {
				const left = execute$1(script.left, scopes, trace);
				const right = execute$1(script.right, scopes, trace);
				if (script.operator === "&&") return left && right;
				else if (script.operator === "||") return left || right;
				throw new NotImplementedError(script, scopes);
			};
			const processMemberExpression = (script, scopes, trace) => {
				...
				const right = script.computed ? execute$1(script.property, scopes, trace) : getName$1(script.property, scopes, trace);
				if (typeGuard.object(left) && typeGuard.definedFunction(left[right])) {
					const func = left[right];
					return execute$1(func.script.arguments[1], [{ self: left }, ...scopes], trace);
				}
				if (typeGuard.LambdaExpression(left)) {
					if (typeGuard.SequenceExpression(script.property)) {
						const args = {};
						let index = 0;
						for (const arg of script.property.expressions) args[`@${index++}`] = execute$1(arg, scopes, trace);
						return execute$1(left.body, [args, ...left.scopes], trace);
					}
					return execute$1(left.body, [{ "@0": right }, ...left.scopes], trace);
				}
				try {
					return processCallExpression({
						type: "CallExpression",
			...
			const processObjectExpression = (script, scopes, trace) => {
				const object = {};
				for (const item of script.properties) object[getName$1(item.key, scopes, trace)] = execute$1(item.value, scopes, trace);
				return object;
			};
			...
			const processSequenceExpression = (script, scopes, trace) => {
				return script.expressions.reduce((_, arg) => execute$1(arg, scopes, trace), void 0);
			};
			const processUnaryExpression = (script, scopes, trace) => {
				const value = execute$1(script.argument, scopes, trace);
				if (script.operator === "-") return UnaryNegation(value);
				else if (script.operator === "+") return UnaryPlus(value);
				else if (script.operator === "~") return BitwiseNOT(value);
				else if (script.operator === "!") return LogicalNot(value);
				throw new NotImplementedError(script, scopes);
			};
			const processUpdateExpression = (script, scopes, trace) => {
				...
				if (script.operator === "--") {
					...
					if (script.prefix) return result;
					else return value;
			...
			const processVariableDeclaration = (script, scopes, trace) => {
				let lastItem;
				for (const item of script.declarations) if (item.init === null) return execute$1(item.id, scopes, trace);
				else if (scopes[0]) lastItem = scopes[0][getName$1(item.id, scopes, trace)] = execute$1(item.init, scopes, trace);
				return lastItem;
			};
			const processors = {
			...
			const execute = (script, scopes, trace, options = { catch: true }) => {
				if (!script || !typeGuard.AST(script)) return;
				if (config.recursionLimit && trace.length > config.recursionLimit) throw new TooMuchRecursionError(script, scopes);
				let result = void 0;
				trace = [...trace, script];
				try {
					const processor = processors[script.type];
					if (processor) result = processor(script, scopes, trace);
				} catch (e) {
				...
				for (const hook of resultHook) result = hook(result);
				return result;
			};
			const initExecute = () => {
				setExecute(execute);
			...
			const processIndex$2 = (script, scopes, object, trace) => {
				...
				if (typeof index === "number") return object[index];
			...
			const processJoin = (script, scopes, object, trace) => {
				...
				if (typeof separator !== "undefined") return object.join(format(separator, "string"));
				return object.join("");
			};
			...
			const processWalk = (script, scopes, object, trace) => {
				...
				let result;
				if (typeGuard.LambdaExpression(processor)) for (const item of object) result = execute$1(processor.body, [{ "@0": item }, ...scopes], trace);
				return result;
			};
			...
			const processDef = (script, scopes, object, trace) => {
				const functionName = (() => {
					if (typeGuard.Identifier(script.arguments[0])) return getName$1(script.arguments[0], scopes, trace);
					if (typeGuard.CallExpression(script.arguments[0])) return getName$1(script.arguments[0].callee, scopes, trace);
					throw new InvalidTypeError("function name must be CallExpression or Identifier", script, scopes);
				})();
				if (typeof functionName !== "string") throw new InvalidTypeError("function name must be string", script, scopes);
				object[functionName] = {
					type: "definedFunction",
					isKari: false,
					script
			...
			const processGetSlot = (script, scopes, object, trace) => {
				...
				if (typeof key !== "string" && typeof key !== "number") throw new InvalidTypeError("[call expression] Object.getSlot: id must be string or number", script, scopes);
				return object[key];
			};
			const processSetSlot = (script, scopes, object, trace) => {
				const key = execute$1(script.arguments[0], scopes, trace);
				if (typeof key !== "string" && typeof key !== "number") throw new InvalidTypeError("[call expression] Object.setSlot: id must be string or number", script, scopes);
			...
			const processIndex$1 = (script, scopes, object, trace) => {
				return object[format(execute$1(script.arguments[0], scopes, trace), "number")];
			};
			...
			(function(module$1) {
				(function(root, factory) {
					if (module$1.exports) module$1.exports = factory();
					...
					function peg$parse(input, options) {
						...
						var peg$f59 = function peg$f59(identifier, argument) {
							return _objectSpread2(_objectSpread2({}, argument), {}, {
								NIWANGO_Identifier: identifier || void 0,
								location: location()
							});
						};
									...
									if (s0 === peg$FAILED) {
										...
										if (s0 === peg$FAILED) {
											...
											if (s0 === peg$FAILED) {
												...
												if (s0 === peg$FAILED) s0 = peg$parseZs();
											}
										}
									}
								}
							...
							if (s0 === peg$FAILED) {
								...
								if (s0 === peg$FAILED) {
									...
									if (s0 === peg$FAILED) {
										...
										if (s0 === peg$FAILED) if (input.charCodeAt(peg$currPos) === 8233) {
											s0 = peg$c10;
											peg$currPos++;
										} else {
											s0 = peg$FAILED;
						...
						function peg$parseIdentifierName() {
							...
							s0 = peg$currPos;
							peg$parse__();
							if (input.charCodeAt(peg$currPos) === 47) peg$currPos++;
							else if (peg$silentFails === 0) peg$fail(peg$e18);
							peg$parse__();
									...
									if (s0 === peg$FAILED) {
										...
										if (s0 === peg$FAILED) {
											...
											if (s0 === peg$FAILED) {
												...
												if (s0 === peg$FAILED) if (peg$r1.test(input.charAt(peg$currPos))) {
													s0 = input.charAt(peg$currPos);
													peg$currPos++;
												} else {
													s0 = peg$FAILED;
						...
						function peg$parseUnicodeCombiningMark() {
							...
							s0 = peg$parseMn();
							if (s0 === peg$FAILED) s0 = peg$parseMc();
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
								result: s0
							};
								...
								if (s0 === peg$FAILED) {
									...
									if (s0 === peg$FAILED) {
										...
										if (s0 === peg$FAILED) {
											...
											if (s0 === peg$FAILED) s0 = peg$parseRegularExpressionLiteral();
										}
									}
								}
							}
						...
						function peg$parseNumericLiteral() {
							...
							if (s1 !== peg$FAILED) {
								...
								if (s3 === peg$FAILED) s3 = peg$parseDecimalDigit();
								peg$silentFails--;
						...
						function peg$parseDoubleStringCharacter() {
							...
							if (s2 === peg$FAILED) {
								...
								if (s2 === peg$FAILED) s2 = peg$parseLineTerminator();
							}
							peg$silentFails--;
							if (s2 === peg$FAILED) s1 = void 0;
							else {
							...
							if (s0 === peg$FAILED) {
								...
								if (s0 === peg$FAILED) s0 = peg$parseLineContinuation();
							}
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
								result: s0
						...
						function peg$parseEscapeSequence() {
							...
							if (s0 === peg$FAILED) {
								...
								if (s0 === peg$FAILED) {
									...
									if (s0 === peg$FAILED) s0 = peg$parseUnicodeEscapeSequence();
								}
							}
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
						...
						function peg$parseCharacterEscapeSequence() {
							...
							if (s0 === peg$FAILED) s0 = peg$parseNonEscapeCharacter();
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
								result: s0
							};
						...
						function peg$parseEscapeCharacter() {
							...
							if (s0 === peg$FAILED) {
								...
								if (s0 === peg$FAILED) {
									...
									if (s0 === peg$FAILED) if (input.charCodeAt(peg$currPos) === 117) {
										s0 = peg$c32;
										peg$currPos++;
									} else {
										s0 = peg$FAILED;
						...
						function peg$parseRegularExpressionFirstChar() {
							...
							if (s0 === peg$FAILED) {
								...
								if (s0 === peg$FAILED) s0 = peg$parseRegularExpressionClass();
							}
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
								result: s0
						...
						function peg$parseRegularExpressionClassChar() {
							...
							if (s0 === peg$FAILED) s0 = peg$parseRegularExpressionBackslashSequence();
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
								result: s0
							};
						...
						function peg$parse__() {
							...
							if (s1 === peg$FAILED) {
								...
								if (s1 === peg$FAILED) s1 = peg$parseComment();
							}
							while (s1 !== peg$FAILED) {
								s0.push(s1);
								s1 = peg$parseWhiteSpace();
							...
							if (s0 === peg$FAILED) {
								...
								if (s0 === peg$FAILED) {
									...
									if (s0 === peg$FAILED) {
										...
										if (s0 === peg$FAILED) s0 = peg$parseObjectLiteral();
									}
								}
							}
							peg$resultsCache[key] = {
						...
						function peg$parsePropertyName() {
							...
							if (s0 === peg$FAILED) {
								s0 = peg$parseStringLiteral();
								if (s0 === peg$FAILED) s0 = peg$parseNumericLiteral();
							}
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
								result: s0
						...
						function peg$parseAssignmentExpressions() {
							...
							s0 = peg$currPos;
							if (input.charCodeAt(peg$currPos) === 59) peg$currPos++;
							else if (peg$silentFails === 0) peg$fail(peg$e70);
							peg$parse__();
							s3 = peg$parseAssignmentExpression();
							if (s3 !== peg$FAILED) {
								s4 = [];
							...
							if (s0 === peg$FAILED) {
								...
								if (s0 === peg$FAILED) {
									...
									if (s0 === peg$FAILED) {
										...
										if (s0 === peg$FAILED) s0 = peg$parseVariableStatement();
									}
								}
							}
							peg$resultsCache[key] = {
						...
						function peg$parsePostfixExpression() {
							...
							if (s0 === peg$FAILED) s0 = peg$parseLeftHandSideExpression();
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
								result: s0
							};
						...
						function peg$parsePostfixOperator() {
							...
							if (s0 === peg$FAILED) if (input.substr(peg$currPos, 2) === peg$c49) {
								s0 = peg$c49;
								peg$currPos += 2;
							} else {
								s0 = peg$FAILED;
								...
								if (s0 === peg$FAILED) {
									...
									if (s0 === peg$FAILED) {
										...
										if (s0 === peg$FAILED) {
											...
											if (s0 === peg$FAILED) if (input.charCodeAt(peg$currPos) === 33) {
												s0 = peg$c53;
												peg$currPos++;
											} else {
												s0 = peg$FAILED;
							...
							if (s0 === peg$FAILED) {
								...
								if (s0 === peg$FAILED) {
									...
									if (s0 === peg$FAILED) {
										...
										if (s0 === peg$FAILED) if (input.substr(peg$currPos, 2) === peg$c67) {
											s0 = peg$c67;
											peg$currPos += 2;
										} else {
											s0 = peg$FAILED;
						...
						function peg$parseBitwiseXOROperator() {
							...
							if (s0 === peg$FAILED) if (input.substr(peg$currPos, 2) === peg$c70) {
								s0 = peg$c70;
								peg$currPos += 2;
							} else {
								s0 = peg$FAILED;
						...
						function peg$parseConditionalExpression() {
							...
							if (s0 === peg$FAILED) s0 = peg$parseLogicalORExpression();
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
								result: s0
							};
													...
													if (s0 === peg$FAILED) {
														...
														if (s0 === peg$FAILED) {
															...
															if (s0 === peg$FAILED) {
																...
																if (s0 === peg$FAILED) if (input.substr(peg$currPos, 2) === peg$c86) {
																	s0 = peg$c86;
																	peg$currPos += 2;
																} else {
																	s0 = peg$FAILED;
						...
						function peg$parseLambdaExpression() {
							...
							if (s0 === peg$FAILED) {
								...
								if (s0 === peg$FAILED) s0 = peg$parseLambdaExpression3();
							}
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
								result: s0
						...
						function peg$parseSourceElement() {
							...
							if (s0 === peg$FAILED) s0 = peg$parseLambdaExpression();
							peg$resultsCache[key] = {
								nextPos: peg$currPos,
								result: s0
							};
			...
			const parseScript = (content, name) => {
				let script = content;
				if (script.startsWith("/")) script = script.slice(1);
				let firstError = void 0;
				for (let i = 0; i < 1e3; i++) try {
					return parserExports.parse(script, { grammarSource: name });
				} catch (e) {
					firstError ??= e;
					if (!(e instanceof parserExports.SyntaxError)) throw e;
					console.info(e.format([{
						source: name,
						text: script
					}]));
					const removed = script.slice(0, e.location.start.offset) + script.slice(e.location.start.offset + 1);
			...
			const processHashCode$1 = (_script, _scopes, object) => {
				let seed = 0;
				for (let i = 0; i < object.length; i++) seed = seed * 31 + object.charCodeAt(i);
				return seed;
			};
			...
			const processMultiply$1 = (script, scopes, object, trace) => {
				return Multiplication(object, execute$1(script.arguments[0], scopes, trace));
			};
			...
			const processSlice = (script, scopes, object, trace) => {
				...
				if (typeof length !== "undefined") return object.slice(format(startIndex, "number"), format(startIndex, "number") + format(length, "number"));
			...
			const processToInteger = (_script, _scopes, object) => {
				if (object.match(/^0[1-7]+/)) return parseInt(object, 8);
			...
			const processAdd = (script, scopes, object, trace) => {
				const value = execute$1(script.arguments[0], scopes, trace);
				if (value === void 0) throw new InvalidTypeError("undefined", script, scopes);
			...
			const processAlternative = (script, scopes, object, trace) => {
				...
				if (object && args.then) return execute$1(args.then, scopes, trace);
				else if (!object && args.else) return execute$1(args.else, scopes, trace);
			};
			const processAnd = (script, scopes, object, trace) => {
				if (!format(object, "boolean")) return object;
				return execute$1(script.arguments[0], scopes, trace);
			};
			const processCall = (script, scopes, _, trace) => {
				if (!script.arguments[0]) throw new InvalidTypeError("function name must be exist", script, scopes);
				const functionName = execute$1(script.arguments[0], scopes, trace);
			...
			const processEquals = (script, scopes, object, trace) => {
				return Equality(object, execute$1(script.arguments[0], scopes, trace));
			};
			const processForEachSlot = (script, scopes) => {
				throw new NotImplementedError(script, scopes);
			};
			...
			const processWhileKari = (script, scopes, _, trace) => {
				let result;
				while (execute$1(script.arguments[0], scopes, trace)) result = execute$1(script.arguments[1], scopes, trace);
				return result;
			};
			...
			const resolvePrototype = (type, name) => {
				if ((type === "object" || type === "array") && prototypeObjectFunctions[name]) return prototypeObjectFunctions[name];
				else if (type === "array" && prototypeArrayFunctions[name]) return prototypeArrayFunctions[name];
				else if (type === "string" && prototypeStringFunctions[name]) return prototypeStringFunctions[name];
				else if (type === "boolean" && prototypeBoolFunctions[name]) return prototypeBoolFunctions[name];
				else if (type === "number" && prototypeNumberFunctions[name]) return prototypeNumberFunctions[name];
				else if (prototypeValueFunctions[name]) return prototypeValueFunctions[name];
				if (type === "array" && prototypeScope.Array[name]) return prototypeScope.Array[name];
				else if (type === "string" && prototypeScope.String[name]) return prototypeScope.String[name];
				else if (type === "boolean" && prototypeScope.Bool[name]) return prototypeScope.Bool[name];
				else if (type === "number" && prototypeScope.Number[name]) return prototypeScope.Number[name];
				else if (type === "object" && prototypeScope.Object[name]) return prototypeScope.Object[name];
			};
	...
	const setIsWide = (val) => isWide = val;
	//#endregion
	...
	*/
	const fonts = {
		win7: { defont: build([fontTemplates.arial]) },
		win8_1: { defont: build([fontTemplates.arial]) },
		win: { defont: build([fontTemplates.arial]) },
		mac10_9: { defont: build([fontTemplates.macGothicPro6]) },
		mac10_11: { defont: build([fontTemplates.macGothic1]) },
		mac: { defont: build([fontTemplates.macGothicPro6]) },
		other: { defont: build([fontTemplates.sansSerif600]) }
	};
	//#endregion
	...
	*/
	...
	//#endregion
	//#region src/contexts/commentHandler.ts
	let handlers = [];
	const resetHandlers = () => {
		handlers = [];
	};
	...
	const getComments = (vpos) => {
		return comments.filter((comment) => comment._vpos <= vpos && comment._vpos > currentTime).map((comment) => ({
			time: comment._vpos,
			comment,
			type: "comment"
		}));
	...
	//#endregion
	//#region src/contexts/objectManager.ts
	let drawObjects = [];
	/**
	* 描画オブジェクトを描画する
	*/
	const draw = () => {
		...
		for (const object of drawObjects) if (object.visible) object.draw();
	};
	/**
	* 描画オブジェクトを追加する
	* @param item
	...
	* @param trace
	*/
	const addQueue = (script, offset, scopes, trace) => {
		queue.push({
			script,
	...
	*/
	const getQueue = (time) => {
		const result = [];
		const newQueue = [];
		for (const item of queue) if (item.time <= time) result.push(item);
	...
	*/
	...
	//#endregion
	//#region src/contexts/scope.ts
	let globalScope;
	let environmentScope;
	const setGlobalScope = (scope) => {
		globalScope = scope;
	};
	const setEnvironmentScope = (scope) => {
		environmentScope = scope;
	};
	//#endregion
	//#region src/contexts/scripts.ts
	let scripts = [];
	/**
	* スクリプトを追加する
	* @param script
	...
	* @param time
	*/
	const getScripts = (time) => {
		const result = [];
		const newScripts = [];
		for (const item of scripts) if (item.time <= time) result.push(item);
		else newScripts.push(item);
		scripts = newScripts;
		return result;
	};
	const resetScripts = () => {
	...
	const getSmoothDuration = (distance) => {
		let step = 0;
		while (distance > 0) {
	...
	const uuid = () => {
		...
		for (let i = 0, len = chars.length; i < len; i++) switch (chars[i]) {
			case "x":
	...
	*/
	var IrObject = class {
		...
		get scale() {
			...
			if (this.mover === "hopping" && currentQueue) return this.options.scale * this.calcMover(currentQueue, this.options.x, "scale");
		...
		set x(val) {
			...
			const lastQueue = this.moverQueue.filter((queue) => queue.vpos === currentTime)[0];
			...
			if (!lastQueue) {
				...
				currentPos = {
					x: lastVal,
					y: lastQueue.current.y
				};
				targetPos = {
					x: input,
					y: lastQueue.target.y
				};
			}
		...
		get __x() {
			...
			const activeWidth = (commentmask ? config.commentLayerWidth : config.videoLayerWidth)[isWide ? "full" : "default"];
			const paddingLeft = isWide ? 0 : (config.stageWidth.full - config.stageWidth.default) / 2;
			const mode = isWide ? "full" : "default";
			const commentMaskOffset = commentmask ? -(config.commentLayerWidth[mode] - config.videoLayerWidth[mode]) / 2 : 0;
			const w = this.__baseWidth;
			if (this.options.posX === "migi") return activeWidth + posX - w + paddingLeft + commentMaskOffset;
			else if (this.options.posX === "hidari") return posX + paddingLeft + commentMaskOffset;
		...
		set y(val) {
			...
			if (!lastQueue) {
				...
				currentPos = {
					x: lastQueue.current.x,
					y: lastVal
				};
				targetPos = {
					x: lastQueue.target.x,
					y: input
				};
			}
			this.__updateMoverQueue(lastQueue, currentPos, targetPos);
		...
		get __y() {
			...
			if (this.options.posY === "ue") return posY;
			else if (this.options.posY === "shita") return config.canvasHeight + posY - h;
		...
		__updateMoverQueue(lastQueue, currentPos, targetPos) {
			const diff = {
				x: targetPos.x - currentPos.x,
				y: targetPos.y - currentPos.y
			};
			if (this.mover === "smooth") this.moverQueue = [{
				current: currentPos,
				target: targetPos,
				diff,
				vpos: currentTime,
				duration: getSmoothDuration(getDistance(currentPos, targetPos))
			}];
			else {
				if (lastQueue) {
		...
		__filterMoverQueue() {
			...
			this.moverQueue = this.moverQueue.filter((item) => item.vpos + item.duration > currentTime);
			if (lastLength !== this.moverQueue.length) this.__modified = true;
			const currentItem = this.moverQueue[0];
			if (this.mover !== "hopping" && this.moverQueue.length > 4 && currentItem) this.moverQueue = [currentItem, ...this.moverQueue.slice(-3)];
		}
		__parsePos() {
			const pos = this.options.pos;
			if (pos.includes("hidari")) this.options.posX = "hidari";
			else if (pos.includes("migi")) this.options.posX = "migi";
			else this.options.posX = "naka";
			if (pos.includes("ue")) this.options.posY = "ue";
			else if (pos.includes("shita")) this.options.posY = "shita";
			else this.options.posY = "naka";
		}
		__updateColor() {
		...
		calcMover(queue, basePos, axis) {
			if (axis === "scale") {
				if (queue && this.mover === "hopping") {
					const _steps = 10;
					const _step = Math.min(Math.floor((currentTime - queue.vpos) / AS_TICK_VPOS), _steps);
					return 1 + (_step * _step - (_steps + 1) * _step + _steps) / -50;
				}
				return 1;
			}
			if (!queue || this.mover === "") return basePos;
			if (this.mover === "simple") return queue.current[axis] + queue.diff[axis] * (currentTime - queue.vpos) / 25;
			else if (this.mover === "hopping") return queue.current[axis] + queue.diff[axis] * (currentTime - queue.vpos) / 25;
			else if (this.mover === "rolling") {
				const _steps = 20;
				const _step = Math.min(Math.floor((currentTime - queue.vpos) / AS_TICK_VPOS), _steps);
				const val1 = 2 * Math.PI / _steps * (_step - 1);
				const val2 = (_step * _step - (_steps + 1) * _step + _steps) / -5;
				return queue.current[axis] + queue.diff[axis] * (currentTime - queue.vpos) / 50 + val2 * (axis === "x" ? Math.cos(val1) : Math.sin(val1));
			} else if (this.mover === "smooth") {
				const stepCount = Math.floor((currentTime - queue.vpos) / AS_TICK_VPOS);
				let pos = queue.diff[axis];
				for (let i = 0; i < stepCount; i++) {
					const step = Math.sign(pos) * (Math.abs(pos) / 28 + 1);
	...
	* @param input
	*/
	const number2color = (input) => {
		return `#${`000000${(input & 16777215).toString(16)}`.slice(-6)}`;
	};
	//#endregion
	//#region src/objects/shape.ts
	const optionTypes$1 = {
		x: "number",
		y: "number",
		z: "number",
	...
	*/
	var IrShape = class extends IrObject {
		...
		__draw() {
			...
			if (this.shape === "rect") context.fillRect(0, 0, this.__width, this.__height);
			else {
				context.beginPath();
				context.ellipse(this.__width / 2, this.__height / 2, this.__width / 2, this.__height / 2, 0, 0, Math.PI * 2);
				context.fill();
			}
		}
		draw() {
			if (!(Math.floor(this.width) > 0 && Math.floor(this.height) > 0)) return;
			...
			if (this.rotation % 360 !== 0) render.drawImage(this, {
				...options,
				rotate: this.rotation
			});
			else render.drawImage(this, options);
		}
	};
	//#endregion
	...
	*/
	const splitContents = (string) => {
		return Array.from(string.match(/[\n\r]|[^\n\r]+/g) || []).map((val) => Array.from(val.match(/[ -~｡-ﾟ]+|[^ -~｡-ﾟ]+/g) || []));
	};
	/**
	* 描画するフォントを処理
	* @param string
	*/
	const getFontIndex = (string) => {
		const regex = {
		...
		if (simsunStrongMatch !== null) index.push({
			font: "simsunStrong",
			index: simsunStrongMatch.index
		});
		const simsunWeakMatch = regex.simsunWeak.exec(string);
		if (simsunWeakMatch !== null) index.push({
			font: "simsunWeak",
			index: simsunWeakMatch.index
		});
		const gulimMatch = regex.gulim.exec(string);
		if (gulimMatch !== null) index.push({
			font: "gulim",
			index: gulimMatch.index
		});
		const gothicMatch = regex.gothic.exec(string);
		if (gothicMatch !== null) index.push({
			font: "gothic",
			index: gothicMatch.index
		});
		return index;
	};
	/**
	...
	*/
	const parse = (string, compat = false) => {
		...
		for (const line of lines) {
			...
			if (firstContent?.font) content.push(...lineContent.map((val) => {
	...
	* @param compat
	*/
	const parseHalfStr = (string, compat) => {
		if (!compat) return {
			type: "normal",
			content: string
		};
		...
		if (buffer || lastChar) result.push({
			type: "text",
			text: buffer + lastChar
		});
		return {
	...
	*/
	const parseFullStr = (string) => {
		...
		if (firstVal.font !== "gothic") return [{
			type: "normal",
			content: string,
	...
	*/
	const measure = (context, comment) => {
		...
		for (const item of comment.content) {
			...
			} else for (const value of item.content) if (value.type === "fill" || value.type === "space") {
	...
	*/
	var IrText = class extends IrObject {
		constructor(_options) {
			...
			if (Math.abs(size) < 10) {
				this.__scale = Math.max(Math.abs(size / 10), .16);
				this.__size = 10;
			} else {
				this.__scale = 1;
		...
		__measure() {
			...
			const kagePadding = this.filter === "kage" ? this.__kageShadowOffset() + Math.ceil(this.__size * .17) : 0;
		...
		__draw() {
			...
			if (this.filter === "kage") {
				...
				context.shadowBlur = this.__size * .17;
			}
				...
				item.content.forEach((part, index) => {
					...
					switch (part.type) {
						case "fill":
							...
							context.fillRect(posX, posY, part.width * this.__size, this.__size * config.lineHeight);
							break;
						case "text":
							if (this.filter === "fuchi") context.strokeText(part.text, posX, posY);
			...
			context.restore();
			if (this.filter === "kasumi") this.kasumi();
		}
		draw() {
		...
		kasumi() {
			...
			canvasBlur050.width = canvas.width * .5;
			canvasBlur050.height = canvas.height * .5;
			const canvasBlur025 = document.createElement("canvas");
			canvasBlur025.width = canvas.width * .25;
			canvasBlur025.height = canvas.height * .25;
			canvasBlur025.getContext("2d")?.drawImage(canvas, 0, 0, canvas.width, canvas.height, 0, 0, canvasBlur025.width, canvasBlur025.height);
			canvasBlur050.getContext("2d")?.drawImage(canvasBlur025, 0, 0, canvasBlur025.width, canvasBlur025.height, 0, 0, canvasBlur050.width, canvasBlur050.height);
			context.clearRect(0, 0, canvas.width, canvas.height);
	...
	const getLatestSnapshot = (vpos) => {
		let maxVpos = -1;
		let lastSnapshot;
		for (const snapshot of snapshots) if (snapshot.vpos <= vpos && maxVpos < snapshot.vpos) {
			maxVpos = snapshot.vpos;
			lastSnapshot = snapshot;
		}
		return lastSnapshot;
	};
	const getLatestSnapshotVpos = (vpos) => {
		const snapshot = getLatestSnapshot(vpos);
	...
	const initResultHook = () => {
		import_niwango_core.default.appendResultHook(resultHook);
	};
	//#endregion
	...
	var CanvasRender = class {
		...
		drawImage(item, options) {
			...
			if (options.rotate !== void 0 && options.rotate % 360 !== 0) {
			...
			} else this._drawImage(canvas, options);
		}
		_drawImage(image, options) {
			if (isDrawOptionA$1(options)) this.renderContext.drawImage(image, options.baseX, options.baseY, options.baseWidth, options.baseHeight, options.targetX, options.targetY, options.targetWidth, options.targetHeight);
			else this.renderContext.drawImage(image, options.targetX, options.targetY);
		}
		apply(clear) {
			clear && this.targetContext.clearRect(0, 0, this.targetCanvas.width, this.targetCanvas.height);
		...
		drawLetterBox() {
			const letterBoxWidth = (config.stageWidth.full - config.stageWidth.default) / 2;
	...
	const isDrawOptionA = (i) => i.baseX !== void 0;
	var DomRender = class {
		constructor(targetElement) {
			...
			wrapperElement.style.transform = `scale(${1920 / config.canvasWidth}, ${1080 / config.canvasHeight})`;
		...
		apply() {
			if (isWide) {
				...
				this.renderElement.style.left = `${(config.canvasWidth - config.stageWidth.default) / -2}px`;
			}
		}
		clear() {
	...
	//#endregion
	//#region src/functions/commentTrigger.ts
	const processCommentTrigger = (script, scopes, _, trace) => {
		const args = import_niwango_core.default.utils.argumentParser(script.arguments, scopes, ["then", "timer"], trace, false);
		addHandler(args.then, scopes, trace, currentTime, args.timer ? Number(import_niwango_core.default.execute(args.timer, scopes, trace)) : void 0);
	};
	//#endregion
	//#region src/functions/drawShape.ts
	/**
	* @関数
	* 図形描画関数
	* @param script
	* @param scopes
	*/
	const processDrawShape = (script, scopes, _, trace) => {
		return new IrShape(import_niwango_core.default.utils.argumentParser(script.arguments, scopes, [
			"x",
			"y",
			"z",
			"shape",
	...
	//#endregion
	//#region src/functions/drawText.ts
	const drawTextArgs = [
		"x",
		"y",
		"z",
	...
	* @param scopes
	*/
	const processDrawText = (script, scopes, _, trace) => {
		return new IrText(import_niwango_core.default.utils.argumentParser(script.arguments, scopes, drawTextArgs, trace));
	};
	/**
	* @関数
	* dt関数 (drawTextのエイリアス、AS側デフォルト値: size=30, filter="fuchi")
	*/
	const processDt = (script, scopes, _, trace) => {
		const args = import_niwango_core.default.utils.argumentParser(script.arguments, scopes, drawTextArgs, trace);
		if (args.size === void 0 || args.size === 0) args.size = 30;
		if (args.filter === void 0) args.filter = "fuchi";
		return new IrText(args);
	};
	//#endregion
	//#region src/utils/mt19937.ts
	const N = 624;
	const M = 397;
	...
	let mti = N + 1;
	const mt = [];
	const __init_genrand = (seed) => {
		mt[0] = seed & 4294967295;
		mti = 1;
		while (mti < N) {
			mt[mti] = 1812433253 * ((mt[mti - 1] || 0) ^ (mt[mti - 1] || 0) >>> 30) + mti & 4294967295;
			mti++;
		}
	};
	const __init_by_array = (seed, length) => {
		let key1 = 0;
		let key2 = 1;
		let range = Math.max(N, length);
		__init_genrand(19650218);
		for (let i = 0; i < range; i++) {
			const value = getNumber(mt[key1]);
			const lastValue = getNumber(mt[key1 - 1]);
			mt[key1] = (value ^ (lastValue ^ lastValue >>> 30) * 1664525) + (seed[key2] || 0) + key2 & 4294967295;
			key1++;
			key2++;
			if (key1 >= N) {
				mt[0] = mt[N - 1] || 0;
		...
		for (let i = 0; i < range; i++) {
			const value = getNumber(mt[key1]);
			const lastValue = getNumber(mt[key1 - 1]);
			mt[key1] = (value ^ (lastValue ^ lastValue >>> 30) * 1566083941) - key1 & 4294967295;
			key1++;
			if (key1 >= N) {
				mt[0] = mt[N - 1] || 0;
				key1 = 1;
	...
	const __genrand_int32 = () => {
		let result;
		if (mti >= N) {
			if (mti === N + 1) __init_genrand(5489);
			let key = 0;
			const index = [0, MATRIX_A];
			while (key < N - M) {
	...
	*/
	const rand = (value) => {
		let seed = 0;
		if (typeof value === "undefined") seed = onload + randCalledCount++;
		else if (typeof value === "number") seed = value;
		else if (typeof value === "string") for (let i = 0; i < value.length; i++) seed = seed * 31 + value.charCodeAt(i);
		else if (typeGuard.comment(value)) seed = (value._vpos * 100 + 1) * value.no;
		const result = mt19937(seed);
		return result < 0 ? -(result + 1) : result;
	};
	const processRand = (script, scopes, _, trace) => {
		if (script.arguments[0]) return rand(import_niwango_core.default.execute(script.arguments[0], scopes, trace));
		return rand();
	};
	//#endregion
	...
	const initDefinedFunctions = () => {
		import_niwango_core.default.appendDefinedFunctions("commentTrigger", processCommentTrigger);
		import_niwango_core.default.appendDefinedFunctions("ctrig", processCommentTrigger);
		import_niwango_core.default.appendDefinedFunctions("drawShape", processDrawShape);
		import_niwango_core.default.appendDefinedFunctions("drawText", processDrawText);
		import_niwango_core.default.appendDefinedFunctions("dt", processDt);
		import_niwango_core.default.appendDefinedFunctions("rand", processRand);
		import_niwango_core.default.appendDefinedFunctions("timer", processTimer);
	};
	//#endregion
	...
	*/
	const setup = () => {
		import_niwango_core.default.resetCore();
		initDefinedFunctions();
		initResultHook();
		resetQueue();
		resetObjects();
		...
		execute(vpos) {
			...
			for (let i = this.lastVpos; i <= vpos; i++) {
				...
				while (tasks.length > 0) {
					...
					try {
						const trace = queue.type === "script" ? [queue.script] : [...queue.trace, {
							__queue: queue.type,
							...queue.script
						}];
						import_niwango_core.default.execute(queue.script, queue.type === "queue" ? queue.scopes : [
							globalScope,
							environmentScope,
